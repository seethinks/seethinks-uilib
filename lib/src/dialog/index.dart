...
enum BeeDialogTheme { android, ios }
...
_createAndroidLayout(BuildContext context, message,
  ...
  if (type == 'confirm') {
    buttons.insert(
        0, {'widget': toTextWidget(cancelButton, 'button'), 'onClick': cancel});
  }

  // 判断主题
  switch (theme) {
    case BeeDialogTheme.ios:
      widget = IosWidget(
          key: widgetKey,
...
class BeeDialog {
  ...
  static Alert alert(BuildContext context) {
    return (message,
        {title = '提示', theme = BeeDialogTheme.ios, button = '确认', onConfirm}) {
